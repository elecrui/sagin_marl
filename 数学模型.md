...
为了实现多智能体间的有效协同，避免智能体为逃避惩罚而故意断开用户连接，设计了基于全局-局部混合机制的奖励函数。系统目标是在满足物理约束的前提下，最小化全网加权任务积压与系统能耗。

**a) 系统惩罚项:**
为了引导智能体满足硬性约束，定义以下基于事件的惩罚：

1.  **约束违规惩罚 ($\Psi_{fail}$):**
    当 UAV 发生碰撞（间距小于 $d_{safe}$）、飞出边界或能量耗尽时，施加严重的生存惩罚：
    $$
...
2.  **拥塞溢出惩罚 ($\Psi_{over}$):**
    为了强化对时延要求的敏感度，当任意级队列发生溢出（丢包）时施加惩罚：
    $$
    \Psi_{over}(t) = -\eta_{drop} \cdot \left( \frac{D_{u}^{drop}(t)}{Q_{scale}} + \sum_{k \in \mathcal{K}_u} \frac{D_{k}^{drop}(t)}{Q_{scale}} \right)
    $$

**b) 性能奖励项:**
为了平衡时延与能耗，定义归一化的负效用函数：

1.  **全网时延抑制:**
    为了防止局部最优，奖励函数包含全网用户的积压状态，而非仅当前关联用户。这迫使 UAV 主动搜索并服务热点区域：
    $$
    r_{delay}(t) = - \frac{1}{K} \sum_{k=1}^{K} \frac{Q_k^{gu}(t)}{Q_{max}^{GU}} - \frac{1}{U} \sum_{u=1}^{U} \frac{Q_u^{relay}(t)}{Q_{max}^{UAV}} - \frac{1}{L} \sum_{l=1}^{L} \frac{Q_l^{comp}(t)}{Q_{max}^{SAT}}
    $$
...
最终奖励 $r(t)$ 定义为加权和：
$$
r(t) = \omega_Q \cdot r_{delay}(t) + \omega_E \cdot r_{energy}(t) + \Psi_{fail}(t) + \Psi_{over}(t)
$$
其中 $\omega_Q, \omega_E$ 为权重因子，且通常 $\eta_{crash} \gg \omega_Q > \omega_E$，以确保安全性优先于性能优化。
