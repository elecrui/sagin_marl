...
def _make_obs(cfg: SaginConfig) -> dict[str, np.ndarray]:
    ...
    own[:7] = np.array([0.1, -0.2, 0.05, -0.05, 0.8, 0.3, 0.4], dtype=np.float32)
    obs = {
        "own": own,
...
def test_actor_can_limit_requested_heads_and_skip_entropy():
    torch.manual_seed(0)
    cfg = SaginConfig(
        users_obs_max=3,
        sats_obs_max=2,
    ...
    with torch.no_grad():
        ...
        full_out = actor.act(obs_tensor, deterministic=False)
        sat_forward = actor.forward(obs_tensor, required_heads=("sat",))
        logprob_parts, entropy_parts = actor.evaluate_actions_parts(
            obs_tensor,
            full_out.action,
            sat_indices=full_out.sat_indices,
            out=sat_forward,
            heads=("sat",),
            need_entropy=False,
        )

    assert sat_only.accel is None
    assert sat_only.bw_action is None
    assert sat_only.sat_select_mask is not None
    assert sat_only.action.shape == (2, cfg.sats_obs_max)
    torch.testing.assert_close(sat_only.logprob, torch.zeros_like(sat_only.logprob))
    assert "mu" not in sat_forward
    assert "bw_alpha" not in sat_forward
    assert set(logprob_parts.keys()) == {"sat"}
    assert entropy_parts == {}
    assert torch.isfinite(logprob_parts["sat"]).all()


def test_forward_with_cached_ctx_matches_direct_forward():
    torch.manual_seed(0)
    cfg = SaginConfig(
        users_obs_max=3,
        sats_obs_max=2,
    ...
    with torch.no_grad():
        cached_ctx = actor.encode_ctx(obs_tensor)
        direct = actor.forward(obs_tensor, required_heads=("sat",))
        cached = actor.forward(obs_tensor, required_heads=("sat",), ctx=cached_ctx)

    torch.testing.assert_close(direct["ctx"], cached["ctx"], rtol=0.0, atol=0.0)
    torch.testing.assert_close(direct["sat_logits"], cached["sat_logits"], rtol=1e-6, atol=1e-6)
    torch.testing.assert_close(
        direct["sat_valid_mask"],
        cached["sat_valid_mask"],
        rtol=0.0,
        atol=0.0,
    )
